...
# Première étape, créer une copie du texte où nous enlèverons les titres de chapitres
...
# Exercice 3: insérez dans le code ci-dessous une expression régulière qui permettront d'éliminer "_" ou "_[mot]_"
# Un indice: il y a trois cas de figure vous pourrez séparer par |
...
# Il convient ordonner la table en fonction des valeurs décroissantes
...
table_filtree_f(mon_dictionnaire_personnel)                       # Insérez votre dictionnaire dans la fonction, puis exécutez le code
...
# Fonction pour créer le dataframe des chapitres à partir du vecteur de lignes
dataframe_chapitres_f <- function(lignes_v) {
  
  # Étape 1: Identifier les indices des titres des chapitres
  titres_indices <- grep("^CHAPITRE", lignes_v)
  
  # Étape 2: Extraire les titres des chapitres
  titres_chapitres <- lignes_v[titres_indices]
  
  # Étape 3: Initialiser une liste pour stocker le texte de chaque chapitre
  texte_chapitres <- list()
  
  # Étape 4: Parcourir les chapitres et assembler le texte
  ...
  # Étape 5: Créer le tableau de données
...
# Nous allons maintenant créer une deuxième colonne avec le texte traité (retrait des mots fonctionnels et bas de casse)
# Fonction pour traiter le texte : conversion en minuscules et suppression des stop words
traitement_texte_f <- function(texte, antidictionnaire) {
  
  # Étape 1: Passer tout le texte en minuscules
  texte_minuscule <- tolower(texte)
  
  # Étape 2: Supprimer la ponctuation et les symboles
  texte_sans_ponctuation <- gsub("\\W", " ", texte_minuscule)
  
  # Étape 3: Supprimer les mots fonctionnels
  # Créer un pattern regex qui correspond à chaque mot de l'antidictionnaire
  pattern <- paste0("\\b(", paste(antidictionnaire, collapse = "|"), ")\\b")
  
  # Remplacer les stop words par une chaîne vide
  texte_traite <- gsub(pattern, "", texte_sans_ponctuation)
  
  # Supprimer les espaces multiples potentiellement créés
  texte_traite <- gsub("\\s+", " ", texte_traite)
  
  # Retourner le texte traité sans espaces superflus en début/fin
...
# La prochaine étape consiste à ajouter une colonne au tableau où nous emmagasinerons tous les m
...
# Il faudrait idéalement surentrainer un modèle de langue, opération qui dépasse le cadre de cet atelier.
# Nous allons utiliser un modèle de langue général et tout de même observer les résultats.
...
# Les sigles d'annotation (Universal Part of Speech Tags) sont définis sur cette page: https://universaldependencies.org/u/pos/index.html
...
# Utilisation du modèle french-gsd pour annoter l'ensemble du roman. Les lemmes seront ensuite emmagasinés dans notre structure de données principale.
